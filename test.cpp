...
void Test3()
{
	...
	const char *body = "original_file_name=test.json&file_contents={\"code\": 200}&page=1";

	happyhttp::Connection conn("127.0.0.1", 80);
	conn.setcallbacks(OnBegin, OnData, OnComplete, 0);

	conn.request("POST", "/save_file.php", headers, (const unsigned char*)body, strlen(body));
